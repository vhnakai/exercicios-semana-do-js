...
const countNumberlessThan501 = (acc, item) => (item < 501 ? ++acc : acc);
const under501Numbers = crazyNumbers.reduce(countNumberlessThan501, 0);

console.log(under501Numbers);
...
const moviesLauchedBefore2000 = tarantinoMovies.filter(
  ({ release }) => release < 2000
);

console.log(moviesLauchedBefore2000);
/*
  05
...
const onlyTvShowsNames = tvShows.map(({ name }) => name);
...
const gamesInCart = cart.reduce((acc, { name }) => `${acc} - ${name}\n`, "");
